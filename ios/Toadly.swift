...
class Toadly: HybridToadlySpec {
    ...
    private var sessionReplayEnabled = false
    private static var sessionReplayService: SessionReplayService?
    ...
    public func setup(githubToken: String, repoOwner: String, repoName: String) throws {
        ...
        // Initialize and start session replay
        Toadly.startSessionReplay()
    }
    
    // Enable or disable session replay
    public func setSessionReplayEnabled(enabled: Bool) throws {
        sessionReplayEnabled = enabled
        
        if enabled {
            Toadly.startSessionReplay()
        } else {
            Toadly.stopSessionReplay()
        }
    }
    
    // Start session replay recording
    private static func startSessionReplay() {
        if sessionReplayService == nil {
            sessionReplayService = SessionReplayService.shared
        }
        sessionReplayService?.startRecording()
    }
    
    // Stop session replay recording
    private static func stopSessionReplay() {
        sessionReplayService?.stopRecording()
    }
    ...
    public func createIssueWithTitle(title: String, reportType: String?) throws {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            // Get session replay data if available
            let sessionReplayData = Toadly.sessionReplayService?.createReplayGif()
            ...
            GitHubService.submitIssue(
                ...
                sessionReplayData: sessionReplayData,
